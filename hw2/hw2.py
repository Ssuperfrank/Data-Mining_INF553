...
if case_num == "1":
    baskets = process_lines.map(lambda x: (x[0], x[1])).distinct().groupByKey().mapValues(list).map(lambda x:(set(x[1])))
    print(baskets.collect())

if case_num == "2":
    baskets = process_lines.map(lambda x: (x[1], x[0])).distinct().groupByKey().map(lambda x:(set(x[1])))
    print(baskets.collect())
