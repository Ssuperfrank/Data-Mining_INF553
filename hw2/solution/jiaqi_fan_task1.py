...
header = data.first()
preData = data.filter(lambda record: record != header).map(lambda record: record.split(","))
if case_number == "1":
    preData = preData.map(lambda x: (x[0], x[1])).groupByKey().mapValues(set).values().cache()
elif case_number == "2":
    preData = preData.map(lambda x: (x[1], x[0])).groupByKey().mapValues(set).values().cache()
else:
    print("error command")
    exit(1)

numBasket = preData.count()
ps = support / numBasket
...
for i in range(1, len(intermediate_result)):
    print(intermediate_result[i][1].__str__().lstrip("[").rstrip("]") + "\n", file=output)
...
for i in range(1, len(final_result)):
    print(final_result[i][1].__str__().lstrip("[").rstrip("]") + "\n", file=output)
